...
Lógica, probabilidad y heurística son tres ideas centrales en la
historia del intelecto humano. A la aspiración de la lógica aristotélica
se agregó a mediados del siglo XVII la teoría de las probabilidades, que
...
[heurística](https://psicologiaymente.com/inteligencia/heuristicos-atajos-mentales-pensamiento),como
recurso que recurre a la imaginación creativa y a la perspicacia de los
seres humanos para descubrir cosas nuevas. Desde entonces el término ha
florecido con matices diversos en todos los campos de la ciencia, la
...
que significa la prueba de sangre positiva, ¿será vampiro el sujeto de quien se
obtuvo esa muestra?. Esta es una manera de estimar la probabilidad de
que el amigo sea en realidad un vampiro, dado el conocimiento y la
evidencia que tenemos a la mano.
...
> Cuando las circunstancias cambian, yo cambio de opinión. ¿Tú que
> haces?"

Si hubiera nueva evidencia disponible, el proceso se aplicaría en
secuencia. Lo que lleva a la siguiente expresión que sugiere como se
puede [*generalizar este proceso de
razonamiento*](https://youtu.be/D7KKlC0LOyw?si=6e_kZ0-1lBDpaTjE). Pero
por ahora ya no ahondaremos más en esto.
...
parecido, es el procedimiento que se sigue en la realidad al efectuar
...
etc. La *prueba de significancia estadística*, ¿entrará en este mismo
marco conceptual?.
...
casos verdaderos (aunque inevitablemente produzca también algunos falsos
...
única manera de razonar en este caso. Podríamos cuestionar que la
ecuación que usamos aquí salio de la nada; aunque quizás la recuerdes de
algún curso previo, de alguna charla interesante por ahí o incluso de lo
que viste con Rosario ¡hace unas semanas!
...
de informar sobre las probabilidades, como hicimos arriba, te digo lo
siguiente:
...
contar la historia mucho más fácil de comprender. Sigamos por este
camino.
...
verdaderamente de vampiros, lo que nos lleva sencillamente a
preguntarnos sobre la fracción que significa esa cantidad de vampiros
entre la totalidad de pruebas positivas.
...
por el camino bayesiano que encontramos antes. La diferencia es que
ahora, no tuvimos que recordar la "formula mágica" de Bayes, nada más
tuvimos que imaginar la situación de otra manera y simplemente contar y
pensar con calma.
...
[Gigerenzer](https://onlinelibrary.wiley.com/authored-by/Gigerenzer/Gerd?pageSize=20&startPage=1) sobre la importancia de la intuición.
...
1.  Una hipótesis es verdadera o falsa, pero no lo sabemos todavía;
2.  Obtenemos un indicio estadístico de la **falsedad** de la hipótesis;
3.  Podemos utilizar el teorema de Bayes para deducir
...
${Pr[señal~detectada | verdadera]=Pr[d | H]= 0.95}$.
...
hipótesis es falsa, es ${Pr[señal~detectada|falsa]= Pr[d | \neg H]= 0.05}$.

Esa es la tasa de falsos positivos, digamos 5%, de la
...
hipótesis resulta ser verdadera. Entonces ${Pr[verdadero]= Pr(H) = 0.01}$.
...
```{webr-r}
# (0.95 * 0.01) / ((0.95 * 0.01) + (1 - 0.95 * (1-0.01)))
Pr_posterior_hallazgo_cierto <- NA
...
implicaciones de la idea en todo tipo de situaciones. Quizás podríamos
considerarlo semejante a lo que Einstein llamaba **experimento
pensado**. Veamos como hacerlo.
...
# fondo de las gráficas evita aparezca un bloque obscuro en su lugar
par(bg = "white")
...
positivos a 1%, ¿hasta cuánto nos permite llevar la probabilidad posterior de
...
-   [ ] ¿Habrá que incrementar siempre el número de veces que se repite el
    tratamiento?
...
seguramente nos *sentiremos con confianza* como para pensar que nuestro modelo es útil para describir el comportamiento del segmento de la naturaleza que
elegimos abstraer.
...
tienen que ver con el fenómeno que estamos analizando lo llamaremos
...
segundo modelo es postular restricciones (supuestos que especifican
valores particulares) sobre los parámetros del primer modelo.
...
::: {.callout-tip title="¿Piensas que sería válido o útil restringir un modelo con valores de algún parámetro con un valor que no sea cero?"}
...
podríamos tener: la del modelo completo**. Llamaremos a este valor,
provisionalmente, el *índice proporcional de error* (**IPE**). Un
...
$$
IPE=\frac{\frac{(E_{C}-E_{R})}{gl_{C}-gl_{R}}}{\frac{E_{C}}{gl_{C}}} \\
...
-   Los distintos modelos abarcan las varias hipótesis nulas que podrían
    plantearse, H<sub>0</sub>, pero también pueden incluir cualquiera de
    las *hipótesis alternativa*, H<sub>A</sub>, que nos interese
    valorar.
-   El procesos de selección de modelos y las características de los
...
Por cierto esta es una prueba "ómnibus", es decir ¡prueba todo (la
...
set.seed(1234) # para fines didácticos: obtener resultados repetibles 
...
Pongamos estos datos en una gráfica simple, según el orden en el que
...
Para referencia podemos pedirle a **R** que nos reporte el cuadro de
...
azar, garantiza al máximo posible las vías de influencias ocultas o
...
sugiere el diagrama, es la única causa que actúa sobre el tratamiento.
Si hubiera una vía de influencia alternativa (*backdoor*), a través de
alguna tercera variable como podría ser en un caso de germinación de
semillas, la luminosidad del sitio o el grado de humedad en el sustrato;
entonces, la aleatorización no sería el único factor que influiría sobre
el tratamiento. El recuadro alrededor de *A* (el proceso de
aleatorización) indica que no existen otros factores actuando sobre *T*,
es decir, *A* es una influencia puramente estocástica. Esta idea y el
proceso de realización de un experimento controlado con aleatorización,
da cuenta con toda claridad del valor de esta forma de realizar estudios
para desentrañar relaciones de causalidad.
...
la media general de la variable *Y*, aunque puede elegirse cualquier
otro valor de referencia que convenga al estudio) y la épsilon da cuenta
del efecto aleatorio inducido por $\fbox{A}$, por lo que es necesario
...
entre sus planteamientos y posturas. En especial Fisher y Neyman
...
### [**¿Es este procedimiento afín al [falsacionismo](https://www.uaeh.edu.mx/scige/boletin/prepa3/n9/r1.html) Popperiano?.**]{style="color:GoldenRod"}
...
![](images/Potencia_estadistica.png){width="700"}
...
muestra como la que tenemos enfrente, si la hipótesis nula considerada fuera
...
Sin embargo, la probabilidad que realmente interesa es la anteriormente mencionada
...
### [¿Recuerdas qué representa cada uno de ellas?]{style="color:GoldenRod"}

### [¿Cuál es la relación entre ambas?]{style="color:GoldenRod"}
...
*clásico*. También queda claro que el [falsacionismo no resulta ser un
camino
inequívoco](https://culturacientifica.com/2013/11/12/la-tesis-duhem-quine-introduccion/),
cosa que ya advertía el mismo Popper, aunque no al extremo de la tesis
Duhem-Quine [@kemeny1952], que sostiene la imposibilidad de hacer
...
el mismo cálculo del error estándar, $s_{e}=\frac{\sigma}{\sqrt{n}}$,
sugiere la solución.

![](images/hipotesis-alternativas.png){width="700"}
...
desempeño de larvas de mariposa que toman una dieta rica en proteínas y
...
ese arreglo mínimo se repetirá 5 veces, así que requeriremos 20 unidades
