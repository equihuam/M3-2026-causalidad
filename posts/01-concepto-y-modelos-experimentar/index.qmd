...
${Pr[señal~detectada | verdadera]=Pr[H|d]= 0.95}$.
...
hipótesis es falsa, es ${Pr[señal~detectada|falsa]= Pr[H|\neg d]= 0.05}$.
...
$$
Pr[H|d]=Pr[Hipótesis|detección]= \frac{Pr[detección|Hipótesis] \cdot Pr(deteción)} {Pr(Hipótesis)} = \\
\\
\frac{Pr[d|H] \cdot Pr[d)} {Pr(d|H] \cdot Pr[d]+ Pr[\neg d|H] \cdot Pr(\neg d)}
\\
$$

```{webr-r}
# (0.95 * 0.01) / ((0.95 * 0.01) + (0.05 * (1-0.01)))
