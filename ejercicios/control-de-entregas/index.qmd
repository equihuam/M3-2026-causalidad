...
# Outlook 365
pacman::p_load(Microsoft365R, dplyr, stringr, DT, flextable)
...
# Prepara la tabka de registro de coreos y su tipo dw contenido
...
for (correo in correos)
{
  ...
  if (str_detect(str_to_lower(asunto), "m3-2026|m3 2026|m3_2026"))
  {
    ...
    } else {anexo <- paste0(asunto, " - sin anexo")}
...
tabla_correo <- tabla_correo |>
  ...
  filter(!str_detect(anexo, "sin anexo")) |> 
  mutate(tipo = if_else(str_detect(str_to_lower(archivo),"ejerc|tarea|descrip|dag|larva|curso"),
                        "ejercicio", "lectura"), 
         ...
         id_num = if_else(str_detect(str_to_lower(archivo),"larva|curso"), "3", id_num)) |> 
  arrange(tipo, id_num)
```
